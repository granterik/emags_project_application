...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.roundabout_sound = pygame.mixer.Sound('audio/roundabout_repulsion.mp3')
        self.roundabout_sound.set_volume(0.9)

        self.flicker_active = False
        self.flicker_next_time = 0
        self.flicker_interval = 400  # Start slow
        self.flicker_decay = 0.8  # Speeds up each flicker
    ...
    def start_flicker(self):
        ...
        self.flicker_next_time = pygame.time.get_ticks() + self.flicker_interval
        self.flicker_interval = 400
        self.flicker_count = 8  # Number of visibility toggles (4 visible-invisible cycles)
...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, type):
        ...
        if type == 'coil_long':
            coil_long_1 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/coil_long_frame1.png').convert_alpha(), 0.2)
            coil_long_2 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/coil_long_frame2.png').convert_alpha(), 0.2)
            self.frames = [coil_long_1, coil_long_2]
            y_pos = 300
        if type == 'coil_short':
            coil_short_1 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/coil_short_frame1.png').convert_alpha(), 0.2)
            coil_short_2 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/coil_short_frame2.png').convert_alpha(), 0.2)
            self.frames = [coil_short_1, coil_short_2]
            y_pos = 300
        if type == 'openswitch':
            openswitch_1 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/openswitch_frame1.png').convert_alpha(), 0.2)
            openswitch_2 = pygame.transform.scale_by(
                pygame.image.load('assets/obstacles/openswitch_frame2.png').convert_alpha(), 0.2)
...
while True:
    ...
    if game_active:
        ...
        if player.sprite.flicker_active:
            now = pygame.time.get_ticks()
            if now >= player.sprite.flicker_next_time:
                player.sprite.visible = not player.sprite.visible
                player.sprite.flicker_count -= 1
                player.sprite.flicker_next_time = now + int(player.sprite.flicker_interval)
                player.sprite.flicker_interval *= player.sprite.flicker_decay  # Speed up flicker

                if player.sprite.flicker_count <= 0:
                    player.sprite.flicker_active = False
                    player.sprite.visible = False
                    print("Flicker finished. Game over.")
                    game_active = False
        ...
        if collided_obstacle:
            ...
            # put effect
                ...
                print('roundabout')
                player.sprite.roundabout_sound.play()
                if player.sprite.sign == 1:
                    # Effect: Character speeds up slightly
                    pass
                else:
                    print('Character sign is negative. Starting flicker.')
                    player.sprite.start_flicker()
                    player.sprite.flicker_timer = pygame.time.get_ticks()
